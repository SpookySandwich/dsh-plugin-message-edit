...
- **附件完整显示**：图片继续交给 DSH 原生相册（可点开原图），文件显示为文件卡片（类型图标、文件名、类型与大小），顺序与发送时一致；编辑与分叉后附件依旧保留。
- **点开文件看内容**：点击消息里的文件卡片，DSH 右侧栏会打开它自己的文档预览（文本 / 代码 / PDF / 图片等按类型渲染），可直接阅读附件内容，无需先下载。
- **编辑时管理附件**：编辑框里每张附件都可单独删除；点回形针或直接把文件拖进编辑框即可添加，文件在后台流式上传并显示进度、失败可重试；提交后新分支按最终附件集合生成。
...
版本 `1.2.0`：编辑框现在可以删除与上传附件（复用 DSH 原生草稿附件轨道与文件上传服务），并修复带文件的用户消息只显示文字、文件卡片被丢弃的问题——遮蔽渲染与宿主一致地渲染全部附件，无法识别的块类型也不再静默丢失。
...
`1.2.0` 已在 DSH `0.1.5-rc.2` 的隔离 Web 环境验证：插件加载、图片与文件附件的显示、编辑时删除与上传附件、编辑与重试、深层分支，以及持久会话的恢复读取。模型回复使用本地测试实现，未调用远程模型服务。
