...
const inject = [
  ...
  'sessionQuery',
  'webServer',
  // Archives version sessions so they stay out of the sidebar without marking
  // them `origin: 'subagent'`, which would fence them from the stop and
  // model-selection APIs. Declared as a plain string because cordis uses array
  // entries as service names directly — an object here would register the key
  // "[object Object]" and inject nothing. archiveVersion still tolerates the
...
async function createVersionAgent(ctx, source, childId, plan, options) {
  ...
  const child = await ctx.agents.create({
    ...
    meta: {
      ...
      // NOTE: deliberately NOT `origin: 'subagent'`.
      //
      // That was used to keep versions out of the sidebar, because the
      // workspace list filters on `origin !== 'subagent'`. But the API proxy
      // fences the same field: `hasApiRemoteSubagentOwner` treats any session
      // with `origin === 'subagent'` as owned by subagent routing and refuses
      // both `session.cancel` and model selection with
      //   agent-busy: session "..." is owned by subagent routing
      // So hiding versions this way made every edited or retried message
      // impossible to stop and impossible to switch models on — the Stop
      // button simply errored.
      //
      // The session-header schema only accepts `origin` as undefined or
      // 'subagent', so there is no third value to hide behind. Versions are
      // hidden by archiving them instead (see archiveVersion): the same
      // sidebar filter also excludes archived ids, archiving does not touch
      // the fence, and the session stays openable by id for version switching.
  ...
  try {
    ...
    await archiveVersion(ctx, childId);
    return child;
  } catch (error) {
    await child.dispose();
    throw error;
...
/**
 * Keep a version out of the sidebar without fencing it.
 *
 * Archiving hides a session from every grouping surface while leaving it
 * openable by id, which is all version switching needs. Failure here is not
 * fatal: a visible extra conversation is a cosmetic problem, whereas failing
 * the edit would lose the user's text.
 */
async function archiveVersion(ctx, sessionId) {
  // The archive set is registry-global, so it lives on workspaceRegistry
  // rather than on an individual workspace.
  const registry = ctx.get('workspaceRegistry');
  if (registry === undefined || typeof registry.archiveSession !== 'function') return;
  try {
    await registry.archiveSession(sessionId);
  } catch (error) {
    // Left visible in the sidebar; the version itself is fine.
  }
}
...
/**
 * Every session in this conversation's version family: the root and all of its
 * descendants, regardless of which branch `sessionId` sits on.
 *
 * Deliberately lighter than `tree()` — it needs identities only, so it skips
 * the per-session version-log reads that projection performs.
 */
async function familySessionIds(ctx, sessionId) {
  ...
  const rootId = trace.complete
    ...
    : trace.ancestors.at(-1)?.header.id ?? sessionId;
  const rootTrace = rootId === sessionId ? trace : await ctx.sessionQuery.traceSession(rootId);
  const ids = new Set([sessionId, rootId]);
  for (const { record } of flattenLineage(rootTrace.target, rootTrace.descendants)) {
