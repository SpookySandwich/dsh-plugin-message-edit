...
async function tree(ctx, sessionId) {
  ...
  const logs = await mapConcurrent(flat, async ({ entry }) => {
    if (entry.ghost || entry.parentId === undefined) return [];
    const record = recordsById.get(entry.id);
    return record === undefined ? [] : versionLog(ctx, record);
  });
  ...
  const versions = flat.map(({ entry, depth }, index) => {
    // A ghost's own version marker survives in its descendants' seeds, so a
    // deleted version still shows what it was — it just cannot be opened.
    let effect;
    let time;
    if (entry.ghost) {
      ...
      const record = recordsById.get(entry.id);
      try {
        const version = record === undefined ? undefined : ownVersionEvent(record.header, logs[index] ?? []);
