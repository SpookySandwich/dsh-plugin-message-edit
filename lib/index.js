...
async function createVersionAgent(ctx, source, childId, plan, options) {
  ...
  const child = await ctx.agents.create({
    ...
    meta: {
      ...
      // Hidden from the sidebar session list (same as a subagent child).
      // Version switching still opens these sessions; they just do not
      // appear as extra conversations on the left.
      origin: 'subagent',
